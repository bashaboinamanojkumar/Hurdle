export function HudrdleLogo({ className = "", size = 32 }: { className?: string; size?: number }) {
  return (
    <svg
      width={size}
      height={size}
...
export function HudrdleLogoFull({ className = "" }: { className?: string }) {
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <HudrdleLogo size={36} />
      <span className="font-heading text-xl font-bold tracking-tight">Hudrdle</span>
    </div>
  )
}
